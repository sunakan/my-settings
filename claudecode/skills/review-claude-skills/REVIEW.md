...
- `disable-model-invocation: true` の場合に description が context に load されない点の注意書きがある（有用な補足）
...
- **`.claude/rules/skill-design.md` 参照**: 「詳細は `.claude/rules/skill-design.md`（SKILL.md 編集時に自動 load）」と本文に書かれているが、そのファイルが存在するか確認が必要（参照先が欠如していると実行時に混乱する）

...
| `context` | 出力先がファイル（調査・生成・更新など）なら `context: fork` を付けているか。付けないと中間処理がメインコンテキストを汚染する。書き込みを含む場合も `context: fork` のみで問題なし。**逆に、出力がユーザーへの会話（評価結果の提示・確認・対話フロー）であれば `context: fork` を付けないのが正しい**（fork すると結果が会話に戻らない） |

...
## 総評
git コミットメッセージを3候補提示してからコミットする、責務が明確なスキル。`disable-model-invocation: true` と `allowed-tools` の絞り込みが適切で、副作用操作のセキュリティ面は良好。`when_to_use` の欠如が唯一の改善余地。
...
- **`context: fork` を付けていないのは正しい**: 候補提示→ユーザー確認→コミット実行という対話フローが必要なため、fork すると結果が会話に戻らず機能しない
